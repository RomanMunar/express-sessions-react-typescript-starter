...
import { githubSignInLink, googleSignInLink, RegisterFields } from '../authFetchers'

const registerForm = ({
  onRegister,
}: {
  onRegister: (body: RegisterFields) => Promise<void>
}) => {
  ...
  const onRegisterSubmit = () => {
    setBody({ email: '', name: '', password: '', passwordConfirmation: '' })
    onRegister(body)
  }
  return (
    <div
    ...
    >
      ...
      <div>
        <h2 className="mt-10 text-xl font-bold">Register</h2>
      </div>
      <div className="space-y-2 w-60">
        ...
        <label htmlFor="name" className="flex flex-col">
          ...
          <input
            onChange={e => {
              setBody(p => ({ ...p, name: e.target.value }))
            }}
            value={body.name}
            name="name"
            type="text"
        ...
        <label htmlFor="confirm-password" className="flex flex-col">
          ...
          <input
            onChange={e => {
              setBody(p => ({ ...p, passwordConfirmation: e.target.value }))
            }}
